...
class WssSubstrateSource(
    ...
    override suspend fun getOrmlTokensAccountData(
        chainId: ChainId,
        assetSymbol: String,
        accountId: AccountId
        ...
        return remoteStorageSource.query(
            chainId = chainId,
            keyBuilder = {
                it.metadata.tokens().storage("Accounts").storageKey(
                    it,
                    accountId,
                    DictEnum.Entry("Token", DictEnum.Entry(assetSymbol, null))
                )
            },
            binding = { scale, runtime ->
                scale?.let { bindOrmlTokensAccountData(it, runtime) }
                    ?: OrmlTokensAccountData.empty()
            }
        )
    }
    ...
    override suspend fun fetchAccountTransfersInBlock(
        ...
        extrinsics.filter { transferWithStatus ->
            ...
            extrinsic.senderId.contentEquals(accountId) || extrinsic.recipientId.contentEquals(
                accountId
            )
        }
    }
